...
export default function Sunglasses() {
  ...
  const [brandGroupsData, setBrandGroupsData] = useState(brandGroups);

  useEffect(() => {
    fetch('http://localhost:8000/api/products?category=sunglasses')
      .then(res => res.json())
      .then(data => {
        if (data && data.length > 0) {
          // If API returns a flat list, we keep the default grouped structure
          // If API already has brand structure, use it
          const imagePool = [DB1, DB2, DB3, DB4, DB5, DB6, LV1, LV2, LV3, LV4, LV5, P1, P2, P3, P4, P5, RB1, RB2, RB3, RB4, RB5];
          const withImages = data.map((p, i) => ({
            ...p,
            image: imagePool[i % imagePool.length]
          }));
          // Organize by brand name or use as-is if already organized
          setBrandGroupsData(Array.isArray(data) && data[0]?.items ? data : brandGroups);
        }
      })
      .catch(err => {
        console.error("Error fetching sunglasses products, using local data:", err);
      });
  }, []);
  return (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {brand.items.map((item) => (
                ...
                >
                  ...
                  <button onClick={() => addToCart({ ...item, id: item._id || item.name })} className="w-full mt-4 bg-white text-black py-2 rounded-lg font-bold text-sm hover:bg-amber-500 hover:text-white transition-colors">ADD TO CART</button>
                </motion.div>
              ))}
            </div>
          </div>
