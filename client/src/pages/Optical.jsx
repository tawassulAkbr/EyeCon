...
export default function Optical() {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/products?category=optical')
      .then(res => res.json())
      .then(data => {
        if (data && data.length > 0) {
          const withImages = data.map((p, i) => ({
            ...p,
            image: [frame1, frame2, frame3, frame4, frame5, frame6, frame7, frame8, frame9, frame10, frame11, frame12, frame13, frame14, frame15, frame16, frame17, frame18, frame19, frame20, frame21, frame22, frame23, frame24, frame25][i % 25]
          }));
      ...
      .catch(err => {
        console.error("Error fetching optical products, using local data:", err);
        setLoading(false);
      });
  }, []);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {products.map((product) => (
            ...
            >
              ...
              <button onClick={() => addToCart({ ...product, id: product._id || product.name })} className="w-full mt-4 bg-white text-black py-2 rounded-lg font-bold text-sm hover:bg-teal-500 hover:text-white transition-colors">
                ADD TO CART
              </button>
            </motion.div>
          ))}
