...
export default function Lens() {
  ...
  const [lensesData, setLensesData] = useState(lenses);

  useEffect(() => {
    fetch('http://localhost:8000/api/products?category=lens')
      .then(res => res.json())
      .then(data => {
        if (data && data.length > 0) {
          const withImages = data.map((p, i) => ({
            ...p,
            image: [l1, l2, l3, l4, l5, l6, l7, l8, l9, l10, l11, l12, l13, l14][i % 14]
          }));
          setLensesData(withImages);
        }
      })
      .catch(err => {
        console.error("Error fetching lens products, using local data:", err);
      });
  }, []);
    ...
    <div className="bg-black min-h-screen pt-32 px-8 pb-20">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {lensesData.map((lens) => (
            <motion.div 
              key={lens._id || lens.id || lens.name}
              whileHover={{ y: -10 }} 
            ...
            >
              ...
              <button onClick={() => addToCart({ ...lens, id: lens._id || lens.name })} className="w-full mt-3 bg-white text-black py-1.5 rounded-lg font-bold text-sm hover:bg-emerald-500 hover:text-white transition-colors">
                ADD TO CART
              </button>
            </motion.div>
          ))}
