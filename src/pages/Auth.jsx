...
export default function Auth() {
  // authMode states can be: 'login' | 'register' | 'forgot'
  ...
  const handleSubmit = (e) => {
    ...
    if (authMode === 'login') {
      alert(`Logging in for: ${formData.email}`)
      ...
      // Validation Check: Addresses match
      if (formData.address.trim() !== formData.confirmAddress.trim()) {
        alert("Error: Shipping addresses do not match.")
        return
      }

      alert(`Registering profile for: ${formData.name}\nContact: ${formData.phone}`)
    } else if (authMode === 'forgot') {
      alert(`Password recovery reset link dispatched to: ${formData.email}`)
      setAuthMode('login')
    }
  }
        ...
        <div className="max-w-md w-full mx-auto space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.div layout className="pt-2">
              <button 
                type="submit"
                className="w-full bg-white text-black font-bold text-xs uppercase tracking-widest py-3 rounded-xl hover:bg-teal-500 hover:text-white transition-all duration-300 flex items-center justify-center gap-2"
              >
                {authMode === 'login' && 'Login'}
                {authMode === 'register' && 'Register Account'}
