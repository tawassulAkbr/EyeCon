...
import { CreditCard, ShieldCheck, Truck, Landmark, CheckCircle, Trash2, ShoppingBag } from 'lucide-react'
...
export default function Cart() {
  ...
  const handlePaymentSubmit = (e) => {
    ...
    // Clear cart context after successful checkout processing simulation
    setTimeout(() => {
            ...
            <div className="pt-2">
              <button
                ...
                className={`w-full font-bold text-xs uppercase tracking-widest py-3.5 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 ${
                  orderProcessed ? 'bg-emerald-600 text-white cursor-default' : 'bg-teal-500 text-white hover:bg-teal-600'
                }`}
              >
                {orderProcessed ? (
                  <>
                    <CheckCircle className="w-4 h-4" />
                    <span>Order Authenticated & Dispatched</span>
                  </>
                ) : (
                  <>
